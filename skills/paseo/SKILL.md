...
`activeTurnBehavior` decides what happens when the target is mid-turn. `"steer"` delivers the prompt into the running turn and leaves it and its subagents alive; `"interrupt"` cancels the turn first. Agent-scoped calls default to `"steer"`, top-level calls to `"interrupt"`. Pass `"interrupt"` only when you mean to stop the work the target is doing.
...
paseo send <agent-id> --steer "<follow-up>"   # do not cancel the turn it is running
paseo ls
